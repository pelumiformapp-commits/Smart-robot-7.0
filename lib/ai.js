...
export async function askVision(imageBase64, mimeType, question) {
  const key = process.env.NVIDIA_NIM_API_KEY;
  if (!key) throw new Error("no NVIDIA key");
      ...
      messages: [
        {
          role: "user",
          content: [
            { type: "text", text: question || "What's in this image? If it's a math or physics problem, solve it fully, step by step, and give a clear final answer." },
