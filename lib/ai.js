...
const BASE_PROMPT =
  ...
  "Talk like a warm, attentive human who genuinely remembers this person and cares how they're doing — not a generic corporate chatbot. " +
  "Natural phrasing and contractions are welcome. Show real interest in follow-ups instead of just answering and stopping. " +
  ...
  "ANTI-FABRICATION RULE (critical, never break this): NEVER guess or invent facts about the person you're talking to — " +
  "not their gender, pronouns, school, course, level, advisor, workplace, or any biographical detail they haven't told you directly. " +
  "NEVER use 'he' or 'she' for the user — address them by name or use 'you'. Only treat something as known if it was actually stated " +
  "(either in this conversation or in the WHAT YOU KNOW section below) — never dress up a guess as a memory. " +
  "CODING RULE: When asked to write, fix, explain, or debug code in ANY programming language (Python, JavaScript, Java, C, C++, C#, Dart, Kotlin, Swift, Go, Rust, PHP, Ruby, SQL, HTML/CSS, etc.), always: " +
  "wrap code in fenced code blocks with the correct language tag (e.g. ```python), keep explanations concise and placed outside the code block, " +
  "point out likely bugs or edge cases when debugging, and ask which language only if it is genuinely ambiguous from context. " +
...
const ALLOWED_LANGUAGES = [
  ...
  "Chinese", "Japanese", "Korean", "Hindi", "Arabic", "Yoruba", "Igbo", "Hausa",
];
...
function normalizeSettings(raw = {}) {
  ...
  return {
    ...
    preferredLanguage: ALLOWED_LANGUAGES.includes(raw.preferredLanguage) ? raw.preferredLanguage : "English",
  };
}

function buildSystemPrompt({ visitorName, isCreator, settings, knownFacts }) {
  ...
  if (s.preferredLanguage && s.preferredLanguage !== "English") {
    prompt += ` LANGUAGE: Reply in ${s.preferredLanguage} by default, unless the user writes to you in a different language — then match their language instead.`;
  }
  if (s.memoryNotes) {
    prompt += ` USER CONTEXT: The user shared this about themselves — personalize replies using it, don't repeat it verbatim, and don't extrapolate beyond it: "${s.memoryNotes.replace(/"/g, "'")}"`;
  }
  if (knownFacts) {
    prompt += ` WHAT YOU KNOW ABOUT THIS PERSON (remembered from past conversations — weave in naturally, never recite as a list or repeat verbatim): ${knownFacts}`;
  }

  prompt += isCreator
    ? " You are talking to Pelumi, your creator. Greet that fact naturally sometimes, with warmth, and address them by name."
    : ` You are talking to ${visitorName || "a guest"}. Address them by name naturally — never guess their gender or any fact about them that wasn't stated.`;
...
async function askVision(imageBase64, mimeType, question, settings) {
  ...
  const visionSystemPrompt =
    ...
    "If any part of the image is genuinely unclear or blurry, say so explicitly rather than guessing at digits or symbols. " +
...
async function generateImageGemini(prompt) {
  ...
  if (!res.ok) {
    const errText = await res.text();
    console.log("Gemini image gen error:", res.status, errText);
    throw new Error(`Gemini image gen ${res.status}`);
  }
  const data = await res.json();
